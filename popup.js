...
function renderList() {
  ...
  if (filtered.length === 0) {
    ...
    showEmpty(q ? `No results for "${q}"` : 'No prompts match the selected category.');
    return;
  }
  ...
  filtered.forEach((prompt, idx) => {
    ...
    li.addEventListener('blur',  () => li.classList.remove('popup-list__item--focused'));
...
function injectPrompt(prompt) {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    chrome.tabs.sendMessage(tabs[0].id, { type: 'INJECT_PROMPT', text: prompt.body }, (response) => {
